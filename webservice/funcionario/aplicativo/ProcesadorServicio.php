...
class ProcesadorServicio {
    ...
    private function crearConexiones() {
        ...
        $cadenaSql = "select * from arka_dbms where nombre='estructura'";
        $resultado = $this->conexionPostgresqlFrame->ejecutarAcceso ( $cadenaSql, 'busqueda' );
         
        $datosConexion->setDatosConexionConsulta($resultado);
        $datosConexion->setDatosConexion("estructura");
        $this->miFabricaConexiones->setRecursoDB ( "postgresql", $datosConexion );
        $this->conexionPostgresqlFrame = $this->miFabricaConexiones->getRecursoDB ( "postgresql" );
	...
	function validarSesion($usuario, $id_dispositivo){
		...
		if (count($resultado) > 0){
			...
			$resultado = $this->conexionPostgresqlFrame->ejecutarAcceso ( $cadenaSql, 'registro' );
	...
	function cerrarSesion($usuario, $dispositivo){
		$log['id_usuario'] = "CONDOR".$usuario;
		$log['accion'] = 'SALIDA';
		$log['id_registro'] = $usuario;
		$log['tipo_registro'] = 'LOGOUT';
		$log['nombre_registro'] = "Cierre de sesión desde ArkaMovil Funcionario, ".$usuario.",".$dispositivo;
		$log['fecha_log'] = date("F j, Y, g:i:s a");
		$log['descripcion'] = "Salida de sistema desde ArkaMovil Funcionario, con el dispositivo: ". $dispositivo;
		$log['host'] = $dispositivo;
		...
		$resultado = $this->conexionPostgresqlFrame->ejecutarAcceso ( $cadenaSql, 'borrar' );

		return $resultado;
		
	}
	
function login($usuario, $dispositivo) {
		...
		if($dispositivo!=''){
			...
			$log['nombre_registro'] = "Autenticacion Exitosa desde ArkaMovil Funcionarios, ".$usuario.",".$dispositivo;
			$log['fecha_log'] = date("F j, Y, g:i:s a");
			$log['descripcion'] = "Ingreso al sistema desde ArkaMovil Funcionarios, con el dispositivo: ". $dispositivo;
			$log['host'] = $dispositivo;
	...
	function elementoFuncionarioPlaca($id_elemento, $usuario, $dispositivo) {	
		...
		if (strcmp($sesion, "sesion_fraudulenta") == 0) {
			...
			$datos = array('id_elemento' => $id_elemento, 
					 	   'funcionario' => $usuario,
			);
			$cadenaSql = $this->miFabricaConexiones->getCadenaSql ( 'elementoInventarioPlaca', $datos );
			$resultado = $this->conexionPostgresqlInventarios->ejecutarAcceso ( $cadenaSql, 'busqueda' );
			return $resultado;
		}
	}
	...
	function consultar_placa($placa, $usuario, $dispositivo) {
		...
		if (strcmp($sesion, "sesion_fraudulenta") == 0) {
			...
			$cadenaSql = $this->miFabricaConexiones->getCadenaSql ( 'elementos_placa', $datos );
			$resultado = $this->conexionPostgresqlInventarios->ejecutarAcceso ( $cadenaSql, 'busqueda' );
	...
	function consultar_placa_imagen($placa, $usuario, $dispositivo) {
		...
		if (strcmp($sesion, "sesion_fraudulenta") == 0) {
			...
			$cadenaSql = $this->miFabricaConexiones->getCadenaSql ( 'consultar_placa_imagen', $datos );
			$resultado = $this->conexionPostgresqlInventarios->ejecutarAcceso ( $cadenaSql, 'busqueda' );
...
// $resultado = $llamada-> login("emmanuel","1235");
...
// $resultado = $llamada->consultar_placa_imagen("165618", "197019", "5a136ddeebffc97");
// $resultado = $llamada->consultar_imagen('3');
// $resultado = $llamada->tipoConfirmacionInventario (0, 1, "dato", 0, 10, "197019", "5a136ddeebffc97");
// $resultado = $llamada->elementosFuncionario('79708124', 'FCMB');
// $resultado = $llamada->elementoFuncionarioPlaca("55758", "197019", "5a136ddeebffc97");
